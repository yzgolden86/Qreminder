...
cd runtimes/node
DATABASE_PATH=./data/qreminder.db \
...
# 3. 把这个用户提升为 admin
sqlite3 ./data/qreminder.db "UPDATE users SET role='admin' WHERE email='admin@local';"
...
pnpm start -- --pb ../../pb_data_dump --target sqlite:///data/qreminder.db --fs ../../runtimes/node/data/assets
