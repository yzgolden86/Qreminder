...
<p align="center">
  <img src="./docs/screenshots/qreminder-dashboard-zh.png" alt="Qreminder 中文仪表盘，展示 20 条开发者订阅、月度支出、近期续费和支出分布" width="100%">
</p>
...
<p align="center">
  <img src="./docs/screenshots/qreminder-subscriptions-zh.png" alt="Qreminder 中文订阅网格，包含筛选、标签、续费状态和服务 Logo" width="100%">
</p>

<p align="center"><strong>统计分析</strong></p>

<p align="center">
  <img src="./docs/screenshots/qreminder-statistics-zh.png" alt="Qreminder 中文统计视图，展示分类支出、付款方式和预算图表" width="100%">
</p>

<p align="center"><strong>续费日历</strong></p>

<p align="center">
  <img src="./docs/screenshots/qreminder-calendar-zh.png" alt="Qreminder 中文续费日历，展示开发者订阅的月度续费事件和预计支出" width="100%">
</p>

<p align="center"><strong>通知方式</strong></p>

<p align="center">
  <img src="./docs/screenshots/qreminder-notifications-zh.png" alt="Qreminder 中文通知设置，展示通知方式列表和邮件通知配置面板" width="100%">
</p>

## 项目简介

如果你同时订了很多工具，Qreminder 可以帮你把它们记清楚：谁什么时候扣费、每月大概花多少、哪些快到期、通知要发到哪里。你可以记录价格、币种、扣费周期、续费日期、付款方式、标签、网站和备注，再用仪表盘、日历、卡片聚合和通知中心查看整体支出和提醒投递情况。

技术形态：

- `packages/client`：Vite + React 19 单页应用，使用 Tailwind 4 + shadcn/Radix，仪表盘与订阅列表合并为单页，卡片管理、续费日历、通知中心各自独立路由，中英文双语。
...
- 通知中心：独立页面汇总「即将发送」与「历史任务」，按状态筛选，单条任务可下钻查看每个收件人的发送结果。
- 看支出：把不同周期折算成月度成本，展示预算使用、分类占比、付款方式占比、停用订阅节省。
- 多币种：可选 Frankfurter 或 FloatRates 汇率源；远端不可用时使用备用汇率。
- 多用户：基于 Better Auth 的邮箱密码登录；管理员可在「设置 → 注册」临时打开注册并配置邮箱白名单（支持 `*@example.com` 通配）。管理员还能通过侧边栏「用户管理」直接增删账号、重置密码。
