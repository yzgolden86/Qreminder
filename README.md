...
> 截图正在配合 2026-05 的视觉重设计重拍。`docs/screenshots/` 目录里的图为旧版本布局，仅作示意；以下章节描述的是当前线上版本的界面与功能。
...
| **多档提醒** | 每条订阅独立 `reminderOffsets`，同日命中合并为一封邮件 |
| **通知渠道** | Workers 走 Resend HTTP API；Node 自托管支持 SMTP、Telegram、Notifyx、Webhook、企业微信机器人、Bark |
...
| **多用户** | Better Auth 邮箱密码登录；管理员可在「设置 → 注册管理」临时打开注册，勾选常用邮箱域名（Gmail / Outlook / QQ / 163 / iCloud 等 12 项）或手动添加白名单 |
...
| [runtimes/node](./runtimes/node/) | Node + better-sqlite3 + nodemailer + node-cron——跑在自己的 VPS（实验中）|
...
| [packages/server](./packages/server/) | 上一代 Go + PocketBase 后端，维护模式（详见末尾 v1 章节）|
...
1. 仓库 **Settings → Environments** 建一个 `cloudflare` environment，按 [docs/CF_GH_ACTIONS_DEPLOY.md §1](./docs/CF_GH_ACTIONS_DEPLOY.md#1-配置-github-secrets--variables) 配 6 个必填 secret（Cloudflare API token、account id、Better Auth secret、APP_URL、Resend API key + 发件人）
...
然后照 [docs/WORKER_DEPLOY.md](./docs/WORKER_DEPLOY.md) 的清单依次：建 D1 + R2 → 填 secrets → 应用 D1 migrations → 构建前端 → `wrangler deploy` → 登录默认 admin。整体约 15 分钟（不含 Resend 域名验证传播时间）。
...
Qreminder 基于 [MIT License](LICENSE) 开源。Copyright © 2026 yzgolden86。
